...
def phone_check(message):  # Уточняем у пользователя чем он с нами поделиться
    if message.text == None:  # Если пользователь нажал кнопку "Поделиться контактом" то текс будет None
        if message.contact.user_id == message.chat.id:  # Проверяем свой ли контакт дал пользователь
            ...
            statup(message)
    elif message.text == "Ввести почту ✉":  # Перекидывает на ввод почты
        mail_check(message)
    elif message.text == 'Ⓜ Главное меню':
        pre_main(message)
    else:
...
def main(message):  # Определяем тип инцидента и уточняем его подтип
    if message.text == 'Наши услуги и стоимость':
        bot.send_message(message.chat.id, '💻 Диагностика двигателя и электронных систем - от 500₽''\n'
                                          '💻 Чип-Тюнинг Автомобилей: увеличение мощности, перевод на Евро-2, '
                                          'отключение: EGR, DPF, EVAP (адсорбер), SCV, Adblue и пр; отключение '
                                          'иммобилайзера; программирование блоков SRS - От 2000₽ ''\n'
                                          '🚘 Замена ГРМ, ремней генератора, помпы (водяного насоса), маслосъёмных '
                                          'колпачков, прокладки клапанной крышки, сальника распредвала, '
                                          'сальника коленвала, прокладки ГБЦ (Головки блока цилиндров).''\n'
                                          '🚘 Ремонт ГБЦ''\n'
                                          '🚘 Регулировка клапанов на автомобилях ВАЗ, Renault, Datsun, Mitsubishi, '
                                          'Great Wall, Honda - от 1000₽''\n'
                                          ...
                                          '‼️Указана минимальная стоимость для перечисленных услуг,цену на конкретный '
                                          'автомобиль и двигатель можно уточнить по телефону или оставить заявку и мы '
                                          'с вами свяжемся‼️')
        ...
        bot.send_message(message.chat.id, 'Обращаю ваше внимание, что на данной стадии формируется автоматическая '
                                          'заявка. Продолжить или вернуться в меню?', reply_markup=keyboard)
        ...
        avito = types.InlineKeyboardButton(text='🌐Avito🌐',
                                           url='https://www.avito.ru/user/2ff12a1824cec4ae791ce715ef78ec7a/profile?src=sharing')
...
def station(message):
    ...
    if message.text == 'Продолжить':
        keyboard = telebot.types.ReplyKeyboardMarkup(True, False)
        keyboard.row('Поехали')
        keyboard.one_time_keyboard = True
        bot.send_message(message.chat.id, 'Оформляем заявку', reply_markup=keyboard)
